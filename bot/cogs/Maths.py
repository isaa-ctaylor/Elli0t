...
class NumericStringParser(object):
    ...
    def eval(self, num_string, parseAll=True):
        self.exprStack = []
        _results = self.bnf.parseString(num_string, parseAll)
...
class Maths(commands.Cog):
    ...
    @commands.command(name = "solve", aliases=["math", "calculate", "calc"])
        ...
        try:
            if "=" in expression:
                ...
                for num, i in enumerate(things):
                    things[num] = nsp.eval(i)

                if things[num].is_integer():
                    things[num] = int(things[num])
                
                answer = all(e == things[0] for e in things)
            
            else:
                answer = nsp.eval(expression)
                
                if answer.is_integer():
                    answer = int(answer)
                
            embed = discord.Embed(
                title="Solved!", colour=discord.Colour.green())
